...
from gh360_interfaces.msg import SetMotorCurrents, SetCurrent, PortStatus, MotorStatus, SetMotorPositions, SetPosition, SetMotorVelocities, SetVelocity


class DoorReset(Node):

    def __init__(self):
        super().__init__('door_reset')
        self.publisher_ = self.create_publisher(SetMotorCurrents, 'motor_goal_current', 10)
        self.pos_publisher_ = self.create_publisher(SetMotorPositions, 'motor_goal_position', 10)
        self.vel_publisher_ = self.create_publisher(SetMotorVelocities, 'motor_goal_velocity', 10)
        ...
        self.create_subscription(
            Bool, 
            'reset', 
            self.cmd_reset_callback, 
            10
        )

        self.client_motor_torque = self.create_client(SetBool, 'motor_set_torque')
    ...
    def cmd_reset_callback(self, msg):
        close_door_msg = msg.data
