...
class DoorEnvObs(Node):
    def __init__(self):
        super().__init__('door_environment_observations')

        self.get_logger().info("Waiting for tf2 buffer...")
        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)

        self.door_env_msg = DoorEnv()
        self.door_env_pub = self.create_publisher(DoorEnv, 'environment_observations', 10)

        self.create_subscription(Float64, 'handle_angle_filtered', self.handle_angle_callback, 10)
        self.create_subscription(PortStatus, 'motor_states', self.hinge_callback, 10)

        timer_period = 0.1 
        self.timer = self.create_timer(timer_period, self.timer_callback)

        # while self.door_env_msg.handle_angle == 0.0 or self.door_env_msg.hinge_angle == 0.0:
        #     self.get_logger().info("Waiting for handle and hinge angle...")
        #     self.spin_once()
        #     pass
