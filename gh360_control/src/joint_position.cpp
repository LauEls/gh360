...
JointPosition::JointPosition(): Node("joint_velocity")
{
    ...
    this->cmd_joint_pos_subscriber_ = this->create_subscription<sensor_msgs::msg::JointState>("cmd_joint_pos", 10, std::bind(&JointPosition::cmd_joint_pos_callback, this, std::placeholders::_1));
    ...
    for (unsigned int i=0; i < msg->position.size(); i++) 
    {
        for (unsigned int j=0; j<this->joints.size(); j++)
        {
            if (msg->name[i] == this->joints[j]->get_joint_name())
            {
                ...
                {
                    this->joints[j]->get_motor(0)->set_present_position(msg->position[i]);
                }
                else
                {
...
void JointPosition::cmd_joint_pos_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
