...
class GH360Teleop(Node):
    ...
    def __init__(self):
        ...
        if self.demonstration_mode == 'random' or self.demonstration_mode == 'gradual_random':
        # if self.demonstration_mode == 'random':
            self.render = False
        else:
            self.render = True
        ...
        self.save_file = '/home/laurenz/phd_project/ros2_gh360_ws/src/gh360/gh360_demonstration/data/spacemouse_demonstrations/robosuite/door_mirror/gh360_sim_door_demonstration_with_variance_v1'
        if self.demonstration_mode == 'random':
            self.save_file = self.save_file+'_random'
        elif self.demonstration_mode == 'gradual_random':
            self.load_file = self.save_file+'.npy'
            self.save_file = self.save_file+'_gradual_random'
        self.save_file_path = self.save_file+'.npy'
        self.config_path = '/home/laurenz/phd_project/TD7/runs/door_mirror/gh360/osc_pose/online/v7_rl_with_demo_with_variance/'
    ...
    def collect_demonstrations(self):
        ...
        self.get_logger().info(f"Collecting {self.demonstration_mode} demonstrations")
        if self.demonstration_mode == 'gradual_random':
            expert_episodes = np.load(self.load_file, allow_pickle=True)
            # expert_episodes = [expert_data[i] for i in range(len(expert_data))]
            self.get_logger().info(f"Loaded {len(expert_episodes)} expert episodes for gradual random mode")
        else:
            expert_episodes = None

        paths = collect_demonstrations(self, self.demonstration_mode, self.num_episodes, expert_episodes=expert_episodes, render=self.render)
        self.get_logger().info(f"{self.demonstration_mode} demonstrations collected")
        self.env.reset()
